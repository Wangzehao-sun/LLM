...
def main_task(config):
    ...
    if config.data.get("se_logits", False):
        ray_cls_with_init = RayClassWithInitArgs(cls=ray.remote(ActorRolloutRefWorker), config=config, role="rollout_ref")
    else:
    ...
    print(f'is_selogits: {config.data.get("se_logits", False)}')
    ...
    #for batch_idx in tqdm(range(num_batch), desc="Generating Batches"):
        ...
        #记录有效输入长度
        valid_input_lengths = attention_mask.sum(dim=1).cpu().numpy()
        batch_dataset["input_lengths"] = valid_input_lengths.tolist()
        ...
        for n_sample in range(config.data.n_samples):
            ...
            #log_probs_padded = wg.compute_log_prob(output_padded)
            output = unpad_dataproto(output_padded, pad_size=pad_size)
            se_log_probs = None
            if config.data.get("se_logits", False):
                print(f"Computing SE logits for sample {n_sample} in batch {batch_idx}.")
                log_probs_padded = wg.compute_ref_log_prob(output_padded)
                log_probs = unpad_dataproto(log_probs_padded, pad_size=pad_size)
                se_log_probs = log_probs.batch.pop("ref_log_prob")
                #batch_logits_lst[n_sample].extend(se_log_probs.cpu().numpy())
            ...
            for i in range(len(output)):
                ...
                if se_log_probs is not None:
                    # 截取有效部分的 logits (对应 response)
                    # se_log_probs[i] 维度通常是 (seq_len,)
                    valid_logits = se_log_probs[i][:valid_response_length]
                    # 转为 float32 的 numpy 数组并保存
                    batch_logits_lst[n_sample].append(valid_logits.detach().float().cpu().numpy())
    ...
    # add to the data frame
        ...
        if config.data.get("se_logits", False):
            ...
            # 计算所有批次的平均logits值
            #avg_logits_length = 
            batch_dataset["se_logits"] = batch_logits_lst
            # 计算所有批次的平均logits值
            
            all_logits = [item for sublist in batch_logits_lst for item in sublist]
            if all_logits:
                    all_values = np.concatenate(all_logits)
                    #裁剪掉极端值
                    clipped_values = all_values[(all_values > -20) & (all_values < 20)]
                    avg_logits = np.mean(np.exp(clipped_values))
                    print(f"Average logits value for batch {batch_idx}: {avg_logits:.4f}")
        ...
        if config.is_eval == True:
            ...
            batch_max_scores = [score["max_score"] for score in score_lst]
            batch_average_score = np.mean(batch_mean_scores)
            batch_average_max_score = np.mean(batch_max_scores)
