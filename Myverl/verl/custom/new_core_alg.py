...
def compute_token_on_off_policy_loss(
    ...
    if target_probs is None:
        ...
        if off_policy_strategy == "no_reshape":
            ...
            elif off_policy_reshape == 'dft':
                #sft_losses = -log_prob
                seq_prob = torch.exp(old_log_prob).detach()
                denominator = torch.where(seq_prob > 0.9, torch.ones_like(seq_prob), torch.clamp(seq_prob, min=0.1))
                off_ratio = off_ratio / denominator
            elif off_policy_reshape == 'masked_sft':
                sft_mask = (off_ratio.detach() > 0.1).float()
                off_ratio = log_prob * sft_mask
            elif off_policy_reshape == 'sft':
                #sft_mask = (off_ratio.detach() > 0.1).float()
                off_ratio = log_prob
            else:
                off_ratio = off_ratio / (off_ratio + 0.1)
            ...
            elif off_policy_reshape == 'luffy':

                off_ratio = off_ratio / (off_ratio + 0.1)
            elif off_policy_reshape == 'se_shaping':
                off_ratio = torch.exp(negative_approx_kl)
                off_ratio = off_ratio/(off_ratio+0.1)
            elif off_policy_reshape == 'se_rlpluss':
                gamma = (1 - torch.exp(log_prob).detach())**(0.5)
                off_ratio = torch.exp(negative_approx_kl)*gamma
            elif off_policy_reshape == 'dynamic_clip':
                prob = torch.exp(log_prob).detach()
                off_ratio = torch.exp(negative_approx_kl)
                #off_ratio = torch.clamp(ratio, max=1.0 / (prob + 1e-8))
                clip_bound = torch.maximum(1.0 / (prob + 1e-8), torch.tensor(1.2, device=prob.device))
                off_ratio = torch.min(off_ratio, clip_bound)
            elif off_policy_reshape == 'dft':
                #sft_losses = -log_prob
                seq_prob = torch.exp(log_prob).detach()
                off_ratio = seq_prob * log_prob
    ...
    elif off_policy_strategy in ["se","se_filter","luffy"]:
        clipped_advantages = torch.clamp(advantages, min=0.0)
        off_pg_losses = -off_ratio * clipped_advantages
        if off_policy_reshape in ['masked_sft','dft','sft']:
            off_pg_losses = -off_ratio
    ...
    #检查一下reward_mask中是否全为1
    #if torch.all(reward_mask == 1.0):
       # print("Warning: reward_mask is all ones!")
    pg_losses = off_pg_losses * prefix_mask*reward_mask + on_pg_losses * (1 - prefix_mask)*reward_mask
...
def compute_token_on_off_sft_loss(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    # ---------------------------
    if on_policy_loss_type == "sft":
        # 根据advantages的正负对on-policy token的sft loss进行clip,正优势为1，负优势为0
        clipped_advantages = (advantages > 0).float()
        on_losses = -(log_prob) * clipped_advantages
    elif on_policy_loss_type == "rl":
        negative_approx_kl = log_prob - old_log_prob
        negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
        ...
        raise ValueError(f"Invalid on_policy_loss_type: {on_policy_loss_type}")
    ...
    if off_policy_loss_type == "sft":
        if off_policy_reshape == 'vanilla':
            ...
            g_sft = torch.sigmoid((sft_gate_threshold - prob) / tau)
            g_rl = 1.0 - g_sft
            ...
            off_sft_losses = -g_sft * sft_advantages * prob
            off_rl_losses = -g_rl * advantages * prob
        ...
        if off_policy_reshape == "batch_mean_norm":
            ...
            # off_ratio -> off_ratio / (off_ratio + 0.1)。prob 带梯度，整形项一并
            # 参与反传，等价于 compute_token_on_off_policy_loss 里的 luffy 分支。
    ...
    on_pg_losses = on_losses * (1.0 - prefix_mask) * reward_mask

    off_pg_loss = verl_F.masked_mean(off_pg_losses, prefix_mask * response_mask * reward_mask)
    if off_pg_loss.isnan().item() is True:
        off_pg_loss = torch.tensor(0.0)
    ...
    pg_losses = off_pg_losses + on_pg_losses
