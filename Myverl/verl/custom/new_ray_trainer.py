...
class NewRayPPOTrainer(RayPPOTrainer):
    ...
    def _prepare_off_policy_from_tgt(self, tgt_inputs_ids: torch.Tensor, gen_batch_off: DataProto, train_batch_size: int, prefix_ratio: float, prefix_lens: list = None) -> DataProto:
        """
    ...
    def _replace_failed_on_policy_with_off_policy(
        ...
        if accuracy_threshold > 0:
            ...
            # 检查每个问题是否全部回答错误 (old behavior: accuracy == 0)
    ...
    def fit(self):
        ...
        logger_extra = Tracking(
            project_name=self.config.trainer.project_name,
            experiment_name=self.config.trainer.experiment_name + "_extra_steps",
            default_backend=self.config.trainer.logger,
            config=OmegaConf.to_container(self.config, resolve=True),
        )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if new_failed_items:
                    failed_questions_buffer.extend(new_failed_items)
    ...
    def _train_step_internal(self, batch_dict, epoch, logger, progress_bar, collect_failures=False, is_failure_recycle_step=False, recycle_sub_step=0):
        ...
        if n_off>0:
            ...
            if 'tgt_input_ids' in batch.batch:
                ...
                gen_batch_off_standard,tgt_list,_ = self._prepare_off_policy_from_tgt(tgt_inputs_ids, gen_batch_off, train_batch_size, prefix_ratio)
            ...
            if  self.config.actor_rollout_ref.rollout.n_prefix>0:
                ...
                if 'se_tgt_input_ids' in batch.batch:
                    ...
                    if 'se_tgt_probs' in batch.batch:
                        se_tgt_probs = batch.batch.pop('se_tgt_probs')
            else:
        ...
        if not manual_repeat:
            if self.config.actor_rollout_ref.actor.policy_loss.loss_mode=='se_filter':
                gen_batch = gen_batch.repeat(repeat_times=n_total, interleave=True)
            else:
        ...
        with marked_timer("step", timing_raw):
            ...
            with marked_timer("gen_off", timing_raw, color="blue"):
                if self.config.actor_rollout_ref.rollout.n_off>0:
                    ...
                    if self.config.actor_rollout_ref.actor.policy_loss.loss_mode=="se_luffy":
                        ...
                        prefix_list = [t[:int(len(t) * prefix_ratio)] for t in tgt_list]
                        gen_batch_off_output_standard= self._build_hybrid_off_policy_output(
                                n_on,
                                self.config.actor_rollout_ref.rollout.n_off,
                                gen_batch,
                                None,
                                prefix_list,
                                train_batch_size,
                    ...
                    if self.config.actor_rollout_ref.rollout.n_prefix>0:
                        # The scheduled ratio, not the config's: a schedule that moves below 1.0
                        # must switch this branch on, or the prefix would be truncated while the
                        # continuation was never generated -- an off row that just stops mid-solution.
                        if prefix_ratio<1.0:
                        ...
                        if se_target == 'standard_dynamic':
                            se_target = "dynamic" if self.global_steps >= 10 else "standard"
                        if se_target == "standard":
                            #se_responses =  se_tgt_input_ids
                            gen_batch_off_output = self._build_hybrid_off_policy_output(
                                n_off,
                                self.config.actor_rollout_ref.rollout.n_se,
                                gen_batch_off,
                                None,
                                se_tgt_list,
                                train_batch_size,
                        ...
                        if se_tgt_probs is not None and se_target != "dynamic":
                            se_old_log_probs = se_tgt_probs.repeat_interleave(n_off, dim=0)
                        else:
            ...
            assert len(gen_batch_output.batch['input_ids']) == train_batch_size * n_total, \
                f"Expected {train_batch_size * n_total} input_ids, but got {len(gen_batch_output.batch['input_ids'])}"
            #打印合并后的prefix_mask列表：
            ...
            # --------------------------------------------------------------------------------
            # START: Collect Failed Questions Logic (User Request)
            # --------------------------------------------------------------------------------
            failed_items = []
            if collect_failures:
                # We use original_index to identify questions
                original_indices = batch.non_tensor_batch['original_index'] # These are per-rollout indices (repeated)
                
                # We need to find indices where ALL rollouts failed.
                # Since we have n rollouts, we group by original_index.
                
                unique_indices = np.unique(original_indices)
                # rewards are in reward_tensor (token level scores).
                # Wait, reward_sum_per_sample has total reward per rollout.
                
                # We need to act on CPU numpy arrays for efficiency
                if isinstance(reward_sum_per_sample, torch.Tensor):
                    reward_np = reward_sum_per_sample.cpu().numpy()
                else:
                    reward_np = reward_sum_per_sample
                
                # original_indices is numpy array?
                if isinstance(original_indices, torch.Tensor):
                    orig_idx_np = original_indices.cpu().numpy()
                elif isinstance(original_indices, list):
                ...
                if failed_original_indices:
                    keys_to_extract = list(batch_dict.keys())
                    for f_idx in failed_original_indices:
                        ...
                        for k in keys_to_extract:
                            ...
                            if isinstance(val, (torch.Tensor, np.ndarray, list)):
                                try:
                                    v = val[f_idx]
                                    # Move to CPU to save memory in buffer
                                    if hasattr(v, 'cpu'):
                                        v = v.cpu()
                                    item[k] = v
                                except IndexError:
            ...
            off_correct_mask = off_policy_mask & (reward_sum_per_sample == success_value)
            metrics['batch/off_correct_avg_response_len'] = (batch.batch['response_mask'][off_correct_mask].sum().item())/(off_correct_mask.sum().item()+1e-6)
            #打印一下se的最终个数和平均response长度
            if self.config.actor_rollout_ref.actor.policy_loss.loss_mode in ['se','se_filter','luffy']:
                metrics['batch/on_solved_none'] = solve_none_on_policy

            else:
                metrics['batch/on_solved_none'] = metrics['batch/solve_none']
            ...
            if rollout_data_dir:
                ...
                with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                    ...
                    # 参考解答（数据集里的 target）：解码 tgt_input_ids 逐行写进 dump。
                    # normal-step（n_off=0）下 tgt_input_ids 未被 pop，batch.repeat 后每行
                    # 已对齐该题的 target；其它路径若已 pop 则跳过（保护）。
                    if "tgt_input_ids" in batch.batch:
                        tgt_dec = self.tokenizer.batch_decode(
                            batch.batch["tgt_input_ids"], skip_special_tokens=True
                        )
                        if len(tgt_dec) == n_rows:
                            dump_infos["target"] = tgt_dec
                    self._dump_generations(
                        inputs=inputs,
                        outputs=outputs,
                        scores=scores,
            ...
            if not is_failure_recycle_step and self.config.trainer.save_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.save_freq == 0):
        ...
        if is_last_step:
            return True, last_val_metrics, failed_items
        
        return False, None, failed_items
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if 'tgt_input_ids' in batch.batch:
                    ...
                    _, tgt_list, _ = self._prepare_off_policy_from_tgt(tgt_inputs_ids, gen_batch_off, train_batch_size, 1.0)
