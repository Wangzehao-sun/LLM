...
class NewDataParallelPPOActor(DataParallelPPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # Check for loss_mode override
        override_loss_mode = data.meta_info.get("loss_mode", None)
        ...
        for epoch in range(self.config.ppo_epochs):
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                for micro_batch_idx, data in enumerate(micro_batches):
                    ...
                    # Determine loss mode: check for override in meta_info, else use config
                    # Note: data was converted to dict/Tensor, so we check the original mini_batch or handle it via meta_info passing
                    # The 'data' variable here is a micro_batch dict. The original 'data' passed to update_policy is the DataProto.
                    # meta_info is stored in the outer scope 'data' (DataProto) which we shadowed with 'data' in loops.
                    # Wait, the outer data is available as 'data' (argument) until shadowed by loop variable.
                    # But Python scoping... 'data' in loop shadows the argument.
                    
                    # Better to extract meta_info at the start of the function.
                    
                    current_loss_mode = override_loss_mode if override_loss_mode else self.config.policy_loss.get("loss_mode", "vanilla")

                    if current_loss_mode == "vanilla":
                        ...
                        append_to_dict(metrics, metrics_data)
                    elif current_loss_mode in ["relift",'hype-sft','sft','rl-sft','sft-sft','none-sft','rl-rl','none-rl']:
                        ...
                        if 'on_loss_contrib' in ret_dict:
                            metrics_data['actor/on_loss_contrib'] = ret_dict['on_loss_contrib'].detach().item()
                        if 'off_loss_contrib' in ret_dict:
                            metrics_data['actor/off_loss_contrib'] = ret_dict['off_loss_contrib'].detach().item()
                        if 'off_sft_loss_contrib' in ret_dict:
                        ...
                        # 绝对贡献(同分母,数值上 on_pos + on_neg + 零优势 == on_loss_contrib)
                        # 正/负 advantage token 的 loss 绝对值比例 ∈ [0,1], sum=1
