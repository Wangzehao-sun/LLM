...
# ---- teacher-API rephraser master switch ----
# true  -> summarize-replace 候选由外部 teacher API 生成（{style_example_1} 用 R 本步
#          对该题的错误 on-policy rollout 就地填入）。
# false -> 回退用本地模型生成这些候选（关闭 teacher）。
# 只是"总开关"；url/model 等参数在 $TEACHER_API_CONFIG 指向的独立 yaml 里。
export TEACHER_API_ENABLE=${TEACHER_API_ENABLE:-true}
export TEACHER_API_CONFIG=${TEACHER_API_CONFIG:-$HOME/LLM/Myverl/verl/custom/config/teacher_api.yaml}
# export TEACHER_API_KEY=sk-...   # 远程 / 需鉴权端点时再设
...
suffix="hype_summarize_mix_"${off_policy_strategy}"_"${off_policy_reshape}


summarize_val_path=$HOME/LLM/Data/deepmath/deepmath_hard_thinkonly_split_summarize_new4_val128.parquet
# Data paths.
train_path=$WORKER_DIR/LLM/Data/deepmath/deepmath_hard_solonly_summarize_teacher.parquet # openr1_seprompt_split_12800.parquet openr1_prompt_rewritten_nothinking_12800_qwen3_4b_token_split_points.parquet
...
python -m verl.trainer.main_ppo_new \
    ...
    +data.recycle_sr_disable_after=100 \
    ...
    +actor_rollout_ref.rollout.summarize_replace=wrong_only \
    +actor_rollout_ref.rollout.accuracy_threshold=0.1 \
    ...
    +actor_rollout_ref.actor.policy_loss.off_distill_coef=0 \
    +actor_rollout_ref.actor.policy_loss.mask_off_inject_group=False \
    +actor_rollout_ref.actor.policy_loss.off_distill_gate=False \
